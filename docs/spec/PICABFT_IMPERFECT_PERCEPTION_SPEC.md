...
The headline N=16 direction fixture is straight-heavy: every approach contains
`2 STRAIGHT / 1 LEFT / 1 RIGHT`, for `8 STRAIGHT / 4 LEFT / 4 RIGHT` overall.
This composition exposes real co-batching without deleting LEFT movements.
LEFT still falls through the unchanged `kSafe` table and is singleton by
construction; there are no protected-LEFT `kSafe` entries.

Four checked-in assignment variants preserve this ratio. `veh0=N-LEFT` and
`veh1=S-STRAIGHT` remain fixed as the reviewed actual conflict pair, while the
remaining identities rotate deterministically. Repetition `r` selects fixture
`r mod 4` before any perception draw, and all six matched rows in that
repetition use the identical fixture and simulation seed. The statistical
ablation crosses honest and `FALSE_DIRECTION` runs with the three modes above,
using 20 paired repetitions per cell (120 strictly sequential runs). The
selected attack kind, target, Byzantine set, fixture, and colluders are logged
during initialization; an adversary may not choose an attack or fixture after
observing sensor noise.

Before the six-row smoke, one honest eligibility-OFF prerequisite MUST show
`mean_batch_size > 1`. The checked-in fixture-0 prerequisite passes with 16
vehicles in 11 batches (`mean_batch_size=16/11=1.45`), all vehicles departed,
and no unsafe co-occupancy. The subsequent six-row smoke also passes: trusting
the false RIGHT declaration admits the reviewed `N-L + S-S` physical conflict,
whereas eligibility ON derives UNKNOWN for the target and all-singleton blocks
co-occupancy. The earlier `4 STRAIGHT / 8 LEFT / 4 RIGHT` 120-run result remains
a LEFT-heavy sensitivity study; its artifacts are not overwritten or relabeled
as the headline traffic mix.
...
2. **Remaining baselines and ablations.** Direction-eligibility-off and
   all-singleton are implemented and their straight-heavy smoke is complete.
   Add the remaining clearly isolated perception-gate-off row, retain the
   existing firewall-off row, and run fixed-time TLS and all-way stop only as
   throughput/delay anchors. Complete the 120-run straight-heavy direction
   profile before freezing its confidence intervals.
...
7. The implemented eligibility-off ablation measures the unsafe authority
   granted by trusting declared direction, while the all-singleton row provides
   the safety-ceiling/throughput-floor anchor; zero perception error remains the
   deterministic limit of the evidence model.
