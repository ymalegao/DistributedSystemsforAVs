...
export VEINS_ROOT="${VEINS_ROOT:-${REPO_ROOT}/.external/veins}"
# NED root, not the C++ include root. OMNeT++ derives a file's package from its
# path below the NED root, so this must be src/v2vbft (where package.ned
# declares org.v2vbft) — exactly as Veins uses src/veins. The compiler's -I
# stays at src/ so "v2vbft/app/..." includes resolve.
export V2VBFT_NED_ROOT="${V2VBFT_NED_ROOT:-${REPO_ROOT}/src/v2vbft}"
export FOURWAY_ROOT="${FOURWAY_ROOT:-${REPO_ROOT}/scenarios/fourway}"
...
_RESDB_ROOT="${RESDB_ROOT:-${REPO_ROOT}/.external/resilientdb}"
...
if [[ -x "./veins" ]]; then
    ...
    if ! has_ned_path_arg "$@"; then
        # NED roots: Veins core, this repo's protocol (src/ is the root so
        # src/v2vbft maps to package org.v2vbft), and the scenario itself.
        cmd+=(-n "${VEINS_ROOT}/src/veins:${V2VBFT_NED_ROOT}:.")
    fi
