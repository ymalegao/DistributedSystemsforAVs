...
# lexicographic node[] ordering.  Leadership is intentionally not encoded
# visually; the leader uses the same yellow color as honest followers.
COLOR_AMBULANCE_IDS=""
COLOR_BYZANTINE_IDS=""
COLOR_SOURCE_LAUNCH=""
COLOR_REPLICA_COUNT=""
...
if [[ -n "${PHASE2_ATTACK_KIND}" &&
      "${PHASE2_ATTACK_KIND}" != "NONE" &&
      "${PHASE2_ATTACK_TARGET}" =~ ^[0-9]+$ ]]; then
    COLOR_BYZANTINE_IDS="${COLOR_BYZANTINE_IDS:+${COLOR_BYZANTINE_IDS},}${PHASE2_ATTACK_TARGET}"
fi
[[ -n "${PHASE2_ATTACK_KIND}" &&
   "${PHASE2_ATTACK_KIND}" != "NONE" &&
...
if [[ -n "${COLOR_SOURCE_LAUNCH}" && -f "${COLOR_SOURCE_LAUNCH_PATH}" ]]; then
    ...
    echo "  SUMO role colors: primary=yellow followers=yellow byzantine=${COLOR_BYZANTINE_IDS:-none} ambulance=${COLOR_AMBULANCE_IDS:-none} (role_color_override.ini)" >&2
    if [[ ${#EXTRA_INI_ARG[@]} -gt 0 ]]; then
