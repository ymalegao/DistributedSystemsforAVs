...
bool CheckPointManager::Wait() {
  ...
  cv_.wait(lk, [&] { return new_data_ > 0 || stop_; });
  return new_data_ > 0;
}

void CheckPointManager::CheckHealthy() {
  uint32_t current_time =
      static_cast<uint32_t>(GetCurrentTime() / 1000000ULL);
...
int CheckPointManager::ProcessStatusSync(std::unique_ptr<Context> context,
  ...
  last_update_time_[sender_id] =
      static_cast<int>(GetCurrentTime() / 1000000ULL);
