...
class ServiceNetwork {
 public:
  ...
  // If enable_network_acceptor is false, ServiceNetwork will not bind any real
  // OS sockets. Inbound messages must be supplied via InjectInboundPacket().
  ServiceNetwork(const ResDBConfig& config,
                 std::unique_ptr<ServiceInterface> service,
  ...
  // Inject a raw inbound packet into the server's receive queue. Intended for
  // deterministic simulation environments (e.g., OMNeT++) where real sockets
  // are disallowed.
  // Returns 0 on success, -1 on invalid input.
  int InjectInboundPacket(const char* buffer, size_t data_len);
 ...
 private:
  ...
  ResDBConfig config_;
  Stats* global_stats_;
  bool enable_network_acceptor_ = true;
};
