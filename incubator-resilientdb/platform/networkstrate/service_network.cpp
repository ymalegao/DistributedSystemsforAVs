...
ServiceNetwork::ServiceNetwork(const ResDBConfig& config,
    ...
    : ServiceNetwork(config, std::move(service), /*enable_network_acceptor=*/true) {}

ServiceNetwork::ServiceNetwork(const ResDBConfig& config,
                               std::unique_ptr<ServiceInterface> service,
                               bool enable_network_acceptor)
    : service_(std::move(service)),
      input_queue_("input"),
      resp_queue_("resp"),
      config_(config),
      enable_network_acceptor_(enable_network_acceptor) {
  struct sigaction sa;
...
int ServiceNetwork::InjectInboundPacket(const char* buffer, size_t data_len) {
  if (buffer == nullptr || data_len == 0) {
    return -1;
  }
  std::unique_ptr<DataInfo> request_info = std::make_unique<DataInfo>();
  request_info->data_len = data_len;
  request_info->buff = new char[request_info->data_len];
  memcpy(request_info->buff, buffer, request_info->data_len);
...
void ServiceNetwork::Run() {
  ...
  if (acceptor_) {
    ...
    // Socketless mode (e.g., OMNeT++): wait until Stop() is called.
    while (IsRunning()) {
