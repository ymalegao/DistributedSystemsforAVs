...
Five ablations for the project presentation. Run on the **current branch** (no merge).
All 4-vehicle (fast) except #5 (18-vehicle emergency scenario). 3 reps each (#5: 3).

**Metric choice:** each ablation uses the metric that actually *shows* its effect —
latency/messages don't move for all of them, so metrics are tailored (see each below).

Regenerate: inside `opp_env`, `benchmarks/ablations/run_ablations.sh <n|all> [reps]`,
then `python3 benchmarks/ablations/analyze_ablations.py`.
