...
#
# --leader <ID>
#   Set replica <ID> as the initial BFT consensus leader by rotating
#   system.initial.view so that <ID> appears first. BFT-SMaRt picks the
#   leader of view v as processes[v % n], so processes[0] = <ID> means
#   replica <ID> leads from the start. config/currentView is deleted before
#   each run so BFT-SMaRt always re-reads system.initial.view fresh.
#   Combine with --byzleader to test an honest non-default leader.
#
# Examples:
...
# which is set here via OMNeT++ ini override — no system.config patching needed.
LEADER_INI=""
if [[ -n "${BFT_INITIAL_LEADER}" ]]; then
    LEADER_INI="${SIM_DIR}/leader_override.ini"
    {
        echo "# Auto-generated by run-omnet-simulation.sh --leader"
        echo "# Do not edit by hand — regenerated each run."
        echo "[General]"
        echo "*.node[*].appl.leaderReplicaId = ${BFT_INITIAL_LEADER}"
    } > "${LEADER_INI}"
    echo "  Set PROPOSE_ALL leader: replica ${BFT_INITIAL_LEADER} (leader_override.ini)"
